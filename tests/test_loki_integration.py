...
class TestLokiIntegration(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        """Start Loki container once for all tests"""
        cls._ctx = start_loki_container()
        cls.loki_container = cls._ctx.__enter__()
        cls.loki_url = cls.loki_container.get_url()
        LOGGER.info(f"Loki URL: {cls.loki_url}")

        # Wait for Loki to be ready
        max_retries = 60
        for i in range(max_retries):
            try:
                response = requests.get(f"{cls.loki_url}/ready", timeout=2)
        ...
        raise RuntimeError("Loki failed to become ready in time")

    @classmethod
    def tearDownClass(cls) -> None:
        """Stop Loki container after all tests"""
    ...
    def test_loki_push_and_query(self) -> None:
        ...
        test_logs = {"streams": [{"stream": {"namespace": "feedmaker", "job": "test"}, "values": [[str(now_ns), '[01/Jan/2024:12:00:00 +0000] "GET /xml/test_feed.xml HTTP/1.1" 200'], [str(now_ns + 1_000_000), '[01/Jan/2024:12:00:01 +0000] "GET /img/1x1.jpg?feed=test_feed.xml&item=123 HTTP/1.1" 200']]}]}

        response = requests.post(f"{self.loki_url}/loki/api/v1/push", json=test_logs, headers={"Content-Type": "application/json"}, timeout=5)
        ...
        params = {"query": '{namespace="feedmaker"}', "start": start_ns, "end": end_ns, "limit": 100}
    ...
    def test_access_log_manager_loki_search(self) -> None:
        ...
        alm = AccessLogManager(loki_url=f"{self.loki_url}/loki/api/v1")
            ...
            "streams": [
                {
                    ...
                    "values": [
                        [str(now_ns), '[22/Dec/2025:12:00:00 +0000] "GET /xml/comic/naver/test_feed.xml HTTP/1.1" 200'],
                        [str(now_ns + 1_000_000), '[22/Dec/2025:12:00:01 +0000] "GET /xml/comic/naver/test_feed2.xml HTTP/1.1" 304'],
                        [str(now_ns + 2_000_000), '[22/Dec/2025:12:00:02 +0000] "GET /img/1x1.jpg?feed=test_feed.xml&item=item123 HTTP/1.1" 200'],
                    ],
                }
            ]
        }
        ...
        params = {"query": '{namespace="feedmaker"}', "start": start_ns, "end": end_ns, "limit": 5000, "direction": "forward"}
    ...
    def test_access_log_manager_search_by_date(self) -> None:
        ...
        test_logs = {
            "streams": [
                {
                    "stream": {"namespace": "feedmaker", "app": "nginx"},
                    "values": [
                        # Feed access logs (RSS XML requests)
                        [str(now_ns), f'[{time_str}] "GET /xml/comic/naver/test_feed1.xml HTTP/1.1" 200'],
                        [
