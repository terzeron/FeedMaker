...
def setup_and_teardown():
    ...
    with (
        ...
        patch(
            "backend.feed_maker_manager.FeedMakerManager.remove_group"
        ) as mock_remove_group,
        patch(
            "backend.feed_maker_manager.FeedMakerManager.get_groups"
        ) as mock_get_groups,
        ...
        mock_get_groups.return_value = (
            [{"name": "test_group", "is_active": True}],
            None,
        )
...
def test_login_rejects_unallowed_email():
    ...
    with (
        patch("backend.main.verify_facebook_token", return_value=True),
