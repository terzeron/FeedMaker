...
from utils.search_manga_site import Site, EleventoonSite, FunbeSite, JoatoonSite, ToonkorSite, XtoonSite, TorrentQqSite, TorrentRjSite, TorrentSeeSite, TorrentTipSite, SearchManager, Method


def test_search_manga_site() -> None:

    def test_site_initialization() -> None:
        print("=== Site Class Initialization Test ===")
        # 테스트용 작업 디렉토리 설정
        ...
        try:
            ...
            assert site.url_prefix == "https://test.com"
            assert site.encoding == "utf-8"
            assert site.render_js is False
            assert site.num_retries == 1
            assert site.headers["Referer"] == "https://test.com"

            print("✓ Site class initialization successful")

        except Exception as e:
            print(f"✗ Site class initialization failed: {e}")
        finally:
            # 테스트 정리
    ...
    def test_url_postfix_setting() -> None:
        print("=== URL Postfix Setting Test ===")
        # 테스트용 작업 디렉토리 설정
        Site.set_work_dir("/tmp/test_work_dir")
        ...
        try:
            ...
            for site, keyword, expected in test_cases:
                ...
                print(f"✓ {site.__class__.__name__} URL postfix setting successful")

        except Exception as e:
            print(f"✗ URL postfix setting test failed: {e}")
        finally:
            # 테스트 정리
            if Path("/tmp/test_work_dir").exists():
                shutil.rmtree("/tmp/test_work_dir")
    ...
    def test_html_extraction() -> None:
        ...
        try:
            ...
            expected = [
                ("테스트 만화 1", "https://test.com/test1"),
                ("테스트 만화 2", "https://test.com/test2")
            ]
            ...
            print("✓ HTML extraction test successful")
    ...
    def test_search_manager() -> None:
        print("=== SearchManager Test ===")
        try:
            search_manager = SearchManager()

            # 기본 초기화 테스트
            assert search_manager.result_by_site == {}
            print("✓ SearchManager initialization successful")

            # 빈 검색 결과 테스트 (실제 사이트 설정 파일이 없어도 동작해야 함)
            try:
                result = search_manager.search("", "존재하지않는키워드", False)
                assert isinstance(result, list)
                print("✓ SearchManager empty search result test successful")
            except Exception as e:
                # 실제 사이트 설정 파일이 없어서 실패하는 것은 정상
                print(f"✓ SearchManager test (expected failure): {e}")
    ...
    def test_utility_functions() -> None:
        ...
        try:
            ...
            # set_url_prefix 테스트
            site.set_url_prefix("https://newtest.com")
            assert site.url_prefix == "https://newtest.com"

            # set_url_postfix 테스트
            site.set_url_postfix("/test/path")
            assert site.url_postfix == "/test/path"
    ...
    def test_individual_site_classes() -> None:
        print("=== Individual Site Classes Test ===")
        # 테스트용 작업 디렉토리 설정
        Site.set_work_dir("/tmp/test_work_dir")
        ...
        try:
            ...
            for site_class in site_classes:
                try:
                    ...
                    print(f"✓ {site_class.__name__} test successful")

                except Exception as e:
                    print(f"✗ {site_class.__name__} test failed: {e}")

        except Exception as e:
            print(f"✗ Individual site classes test failed: {e}")
        finally:
            # 테스트 정리
            if Path("/tmp/test_work_dir").exists():
                shutil.rmtree("/tmp/test_work_dir")
    ...
    def test_site_specific_functionality() -> None:
        print("=== Site-Specific Functionality Test ===")
        # 테스트용 작업 디렉토리 설정
        Site.set_work_dir("/tmp/test_work_dir")
        ...
        try:
            ...
            test_json_content = 'var test_data = [{"t": "테스트 만화", "x": "/test/path"}];'
            ...
            print("✓ Site-specific functionality test successful")

        except Exception as e:
            print(f"✗ Site-specific functionality test failed: {e}")
        finally:
            # 테스트 정리
            if Path("/tmp/test_work_dir").exists():
                shutil.rmtree("/tmp/test_work_dir")
