...
class HeadlessBrowser:
    ...
    def __init__(self, *, dir_path: Path = Path.cwd(), headers: Optional[dict[str, str]] = None, copy_images_from_canvas: bool = False, simulate_scrolling: bool = False, disable_headless: bool = False, blob_to_dataurl: bool = False, timeout: int = 60, wait_until: str = "domcontentloaded") -> None:
        ...
        self._profile_dir: str = os.path.join(tempfile.gettempdir(), "cloakbrowser-profiles", group_hash)
    ...
    def _cleanup_cached_session(cls) -> None:
        ...
        if cache:
            # cloakbrowser patches context.close() to also stop its Playwright instance,
            # so a single close() call handles both layers.
            try:
                cache["context"].close()
            except Exception:
                try:
    ...
    def _launch_session(self) -> dict[str, Any]:
        ...
        # We keep the key for shape compatibility with the playwright variant.
        ...
        try:
            ...
            if session is not None:
                ...
                try:
                    ...
                    LOGGER.warning("Cached cloakbrowser session is no longer responsive, invalidating cache")
                    self._cleanup_cached_session()
                    session_valid = False
                if session_valid:
                    try:
