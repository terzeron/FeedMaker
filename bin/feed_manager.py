...
class FeedManager:
    ...
    def get_feed_name_list_url_count_map(cls) -> dict[str, FeedUrlCountInfo]:
        ...
        with DB.session_ctx() as s:
            ...
            for row in rows:
                feed_name_list_url_count_map[row.feed_name] = FeedUrlCountInfo(feed_name=row.feed_name, feed_title=row.feed_title, group_name=row.group_name, count=row.url_list_count)
    ...
    def get_element_name_count_map(cls) -> dict[str, ElementCountInfo]:
        ...
        with DB.session_ctx() as s:
            for row in s.query(ElementNameCount).all():
                element_name_count_map[row.element_name] = ElementCountInfo(element_name=row.element_name, count=row.count)
    ...
    def get_feed_name_public_feed_info_map(cls) -> dict[str, PublicFeedInfo]:
        ...
        with DB.session_ctx() as s:
            ...
            for row in rows:
                feed_name_public_feed_info_map[row.feed_name] = PublicFeedInfo(feed_name=row.feed_name, feed_title=row.feed_title, group_name=row.group_name, file_size=row.file_size, num_items=row.num_items, upload_date=row.upload_date)
        ...
        with DB.session_ctx() as s:
            ...
            for row in rows:
                feed_name_progress_info_map[row.feed_name] = FeedProgressInfo(
                    feed_name=row.feed_name, feed_title=row.feed_title, group_name=row.group_name, current_index=row.current_index, total_item_count=row.total_item_count, unit_size_per_day=row.unit_size_per_day or 0.0, progress_ratio=row.progress_ratio or 0.0, due_date=row.due_date
                )
    ...
    def get_feed_info(cls, group_name: str, feed_name: str) -> Optional[SingleFeedInfo]:
        ...
        return SingleFeedInfo(
            ...
            collection_info=CollectionDetail(collect_date=feed.collect_date, total_item_count=feed.total_item_count),
            public_feed_info=PublicFeedDetail(public_feed_file_path=feed.public_feed_file_path, file_size=feed.file_size, num_items=feed.num_items, upload_date=feed.upload_date),
            progress_info=ProgressDetail(current_index=feed.current_index, total_item_count=feed.total_item_count, unit_size_per_day=feed.unit_size_per_day, progress_ratio=feed.progress_ratio, due_date=feed.due_date),
        )
