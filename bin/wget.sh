...
verbose_opt=${default_verbose_opt}
...
OPTS=`/usr/local/bin/getopt -o v -l spider download render-js ua referer -- "$@"`
...
if [ "${render_js}" == true ]; then
	...
	cmd="wget -O - \"${verbose_opt}\" \"${timeout_opt}\" \"${ua_opt}\" \"${cert_opt}\" \"${cookie_opt}\" \"${referer_opt}\" \"${url}\""
fi
