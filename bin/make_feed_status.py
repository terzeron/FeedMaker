...
def main() -> int:
    ...
    if not create_feed_status_table(c):
        return -1

    if not update_htaccess_status(c):
        return -1
    if not update_http_request_status(c):
        return -1
    if not update_public_html_status(c):
        return -1
    if not update_feedmaker_status(c):
        return -1
