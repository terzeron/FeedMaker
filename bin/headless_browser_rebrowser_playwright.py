...
except ImportError:  # pragma: no cover - exercised when rebrowser-playwright is unavailable.
    sync_playwright = None


class HeadlessBrowserRebrowserPlaywright(HeadlessBrowserBase):
    """Playwright-compatible Chromium engine using Rebrowser's patched driver."""

    COOKIE_FILE = ENGINE_COOKIE_FILES["rebrowser_playwright"]
    ...
    def make_request(self, url: str, download_file: Optional[Path] = None) -> str:
        try:
            ...
            LOGGER.error("rebrowser-playwright request failed for '%s': %s", url, e)
            return ""
    ...
    def login(self, config: dict[str, str]) -> bool:
        try:
            ...
            LOGGER.error("rebrowser-playwright login failed: %s", e)
            return False

    def _launch_session(self) -> dict[str, Any]:
        if sync_playwright is None:
            raise ImportError(
                "rebrowser-playwright is not installed; run `pip install rebrowser-playwright` "
                "and `rebrowser_playwright install chromium`"
            )
        ...
        for resource in (context, browser):
            if resource is not None:
                try:
                    ...
                    LOGGER.debug("rebrowser-playwright cleanup failed: %s", e)
        if manager is not None:
            try:
                manager.stop()
            except Exception as e:  # pragma: no cover - driver process dependent.
                LOGGER.debug("rebrowser-playwright manager cleanup failed: %s", e)
